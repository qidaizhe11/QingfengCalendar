...
int CBaseSocket::Listen(const char *server_ip, uint16_t port, callback_t callback,
                        void *callback_data)
{
...
//        LOG__(NET, _T("scoket failed, err_code=%d"), _GetErrorCode());
        return NETLIB_ERROR;
    }
...
net_handle_t CBaseSocket::Connect(const char *server_ip, uint16_t port, callback_t callback,
                                  void *callback_data)
{
...
bool CBaseSocket::Shutdown(int nHow)
{
...
//        LOG__(NET, "Accept fd=%d from %s:%d", fd, ip_str, port);
